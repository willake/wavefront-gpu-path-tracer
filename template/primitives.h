...
class Sphere
{
  public:
    Sphere() = default;
    Sphere(int idx, float3 p, float r) : pos(p), r2(r * r), invr(1 / r), objIdx(idx)
    {
    }
    void Intersect(Ray &ray) const
    {
        ...
        if (c > 0)
            return; // we're outside; safe to skip option 2
  ...
  public:
    ...
    float3 GetAlbedo(const float3 I) const
    {
        if (N.y == 1)
        {
            ...
            if (ix == 98 && iz == 98)
                ix = (int)(I.x * 32.01f), iz = (int)(I.z * 32.01f);
            if (ix == 94 && iz == 98)
                ix = (int)(I.x * 64.01f), iz = (int)(I.z * 64.01f);
  ...
  public:
    ...
    void Intersect(Ray &ray) const
    {
        ...
        if (tmin < tmax)
            ...
            {
                if (tmin < ray.t)
                    ray.t = tmin, ray.objIdx = objIdx;
            }
            else if (tmax > 0)
            {
                if (tmax < ray.t)
                    ray.t = tmax, ray.objIdx = objIdx;
            }
#else
    ...
    float3 GetNormal(const float3 I) const
    {
        ...
        if (d1 < minDist)
            minDist = d1, N.x = 1;
        if (d2 < minDist)
            minDist = d2, N = float3(0, -1, 0);
        if (d3 < minDist)
            minDist = d3, N = float3(0, 1, 0);
        if (d4 < minDist)
            minDist = d4, N = float3(0, 0, -1);
        if (d5 < minDist)
            minDist = d5, N = float3(0, 0, 1);
