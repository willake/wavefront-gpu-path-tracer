...
__kernel void finalize(__global uint *finalPixels, __global float4 *accumulator, __global float4 *Es, float scale,
                       uint isDebugging)
{
    ...
    if (isDebugging)
    {
        finalPixels[index] = RGB32FtoRGB8(Es[index]);
        return;
    }

    accumulator[index] += Es[index];
